...
Một số biện pháp được dùng để tránh brute force nhưng vẫn bị khai thác:
|Tên biện pháp|Cơ chế hoạt động|Lỗ hổng|Cách bypass|VD|
|---|---|---|---|---|
|IP blocking|Nếu user nhập sai mật khẩu N lần -> Block IP|Bộ đếm số lần sai reset khi người dùng đăng nhập thành công.|Attacker chỉ cần chèn credential đúng của chính attacker vào wordlist.|Cho max 3 lần sai mật khẩu => 2 lần dùng acc victim xen kẽ 1 lần dùng acc cá nhân hợp pháp cho tới khi brute force thành công password victim|
